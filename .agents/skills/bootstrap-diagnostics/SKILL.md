...
- `MISSING_MANUAL: <tool> (instructions: <url or remediation text>)` - tell the captain why the tool is required and give them the printed instructions, but do not pass the tool to `bin/fm-bootstrap.sh install`; wait for the captain to complete the manual installation, then rerun session start to confirm the dependency is present.
  For `jq`, this also covers an installed build that terminates output lines with CRLF (a native Windows jq): firstmate's own scripts stay defended through `fm_jq` (`bin/fm-jq-lib.sh`), so fleet work may continue, but crewmate project work and other tools on the same PATH are exposed until the captain replaces the build.
