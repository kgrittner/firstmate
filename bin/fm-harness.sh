...
detect_own() {
  ...
  # Layer 2: walk the parent chain and match the command name. fm-proc-lib.sh
  # owns the walk, including the Windows procfs+CIM fallback where ps -o is
  # unavailable and the harness is a native process outside the MSYS ancestry.
  local own
  if own=$(fm_proc_harness_name); then
    echo "$own"
    return
  fi
  echo unknown
