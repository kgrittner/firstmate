...
# An alternation of literal glyphs, NOT a bracket expression: under the plain C
# locale grep matches a bracket expression byte-wise, and every box-drawing
# character shares the UTF-8 lead byte 0xE2 with these glyphs, so '^[❯›]' made
# a composer's own bottom border row match as a bare prompt. Literal multibyte
# strings match byte-exactly in every locale.
